...
** Implement changes of updated cluster library

   - OK r-pitch-pitch has added arg
     format: :pitch

   - ? rule on dur + pitch: start time accessible in rule

   - meter domain: allows for irregular beat durations -- extend example on metric accents


** Consider revising profile rules (or add alternative implementations)
